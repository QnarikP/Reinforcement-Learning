...
def sarsa(action_value_estimates, expected=False, step_size=step_size):
    ...
    while state != goal:
        ...
        # use Expected SARSA
            ...
            best_actions = np.argwhere(next_action_value_estimate == np.max(next_action_value_estimate))

            # calculate the expected value of next state
            for atn in actions:
                if atn in best_actions:
                    target += ((1.0 - exploration_probability) / len(best_actions) + exploration_probability / len(actions)) * action_value_estimates[next_state[0], next_state[1], atn]
                else:
                    target += exploration_probability / len(actions) * action_value_estimates[next_state[0], next_state[1], atn]
