...
def train(epochs: int, print_every_n: int = 500):
    ...
    for i in range(1, epochs + 1):
        ...
        if i % print_every_n == 0:
            print(f"Epoch {i}: player 1 win rate = {(player1_win / i):.02f}, player 2 win rate = {(player2_win / i):.02f}")
...
def compete(turns):
    ...
    print(f"{turns} turns: player 1 win = {(player1_win / turns):.02f}, player 2 win = {(player2_win / turns):.02f}")
...
def play():
    ...
    while True:
        ...
        # Check which player is the winner
        if winner == rl_player.symbol:
            print("You lose!")
        elif winner == human_player.symbol:
            print("You win!")
        else:
            print("It is a tie!")
